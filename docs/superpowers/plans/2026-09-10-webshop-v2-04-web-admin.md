...
- ⚠️ **My claim that every `AdminHeader` story must paint an ink background is FALSE, measured in Task 2.** The bar paints its own opaque `bg-ink`, and axe resolves the background from the ancestor chain without ever reaching the story canvas — removing the decorator left all eight stories green. The claim cuts the other way instead: with ink painted behind it, **losing `bg-ink` from the component is invisible to axe**, and only an explicit background assertion catches it.
- **THE FOCUS RING INVERTS TOO, and this one is a real failure.** Task 2 measured `TextInput`'s accent ring at 5.58:1 on paper and **2.81:1 on ink** — under SC 2.4.11's 3:1 — and **1.76:1** against an `opacity-60` control. Every focusable control placed on the dark bar needs a paper ring (15.69:1, or 6.30:1 through 60% opacity), not the branch's accent one. Applying the shop's ring by reflex is the same mistake as applying the shop's opacity rule, in the opposite direction, with a genuine accessibility failure at the end of it.
- Three of the extract's numbers were off and Task 2 corrected them by measurement: `.4` is **3.51:1**, not 3.36; `.5` is 4.76 and `.55` is 5.49. All decisions stand.
...
**Task 5 — `SpecsEditor`, `PhotosEditor`.** Both edit arrays and neither may hold state. ⚠️ **My identity claim was wrong and Task 5 measured it:** with CONTROLLED inputs, deleting row 2 does not clear row 3's text — the value is re-rendered from props. What a bare index key keeps is the CONTROL, so the delete button that was just clicked stays focused and re-points at the next row. Qualifying the key with the row count is enough; synthetic identity would have to be minted by the container and is not needed. See the shared constraints. Photos: the upload is multipart, `alt` is bilingual, and reordering is a `PUT` of the whole `photos` array where every key must already belong to the product. A new product has no id, so photos are disabled with the spec's hint.

⚠️ **Two things Task 5 settled that later tasks inherit.** (1) **The photos section is not editing a draft.** Adding and removing a photo are IMMEDIATE server calls that each answer with the whole product, while the ORDER and the ALT text ride on the product's own `PUT` — so the container must reseed its photo list from every photo response, or the next save is a 400 keyed `{ photos: ['must_match_existing'] }`. (2) **A wholly blank spec row is dropped before submit** (four `.min(1)` fields per row would otherwise make a brand-new product unsaveable on open, since the design seeds one blank row), which RENUMBERS every zod path behind it — `SpecsEditor` maps display rows onto submit indexes for exactly this reason, and any other consumer of `specs` errors has to do the same.
...
- **Copy Augusto owes, added by Task 5.** The design draws none of these sentences and the spec gives only the English of the first, so the pt-BR is invented: `Salve o produto antes de adicionar fotos.` (spec:204's "Save first to add photos"), `Linhas em branco não são salvas.`, `Máximo de 12 linhas.`, `Nenhuma foto ainda.`, `Enviando a foto…`, `“{{name}}” passa de {{max}} MB. Mande uma foto menor.` and `Não consigo ler “{{name}}”. Mande um JPEG, PNG ou WebP.`
- **Two limit constants duplicated outside `@shop/shared`, for Task 9's sweep.** `MIN_PRICE_CENTS` and `MAX_SPECS` are checked copies — a story parses the boundary through `productInputSchema`, so moving the limit upstream reddens a story. `MAX_PHOTO_BYTES` is NOT: multer's `limits: { fileSize: 8 * 1024 * 1024 }` lives in an Express route no web test can import, so that 8 MB can drift silently. All three belong beside the schemas.
