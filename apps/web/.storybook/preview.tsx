...
import { interceptableAnchor, routeTargetOf } from '../src/app/anchors'
...
// Storybook has no router, so a real `<a href>` in a story is a live link. In the preview iframe
// a click leaves the story; under the vitest browser project it navigates the RUNNER's own page
// out from under itself, which is a hang or a torn-down suite rather than a red assertion. The
// spec (line 181) makes this a global decorator.
//
// It does not re-decide which clicks those are: `interceptableAnchor` is the app's own rule,
// imported from `src/app/anchors.ts` and shared with `LinkInterceptor`. That is what makes
// `AnchorGuard.stories.tsx` evidence about the app rather than about this file — break the rule
// and the stories go red in Chromium alongside the unit tests. This was a second copy of the rule
// until Task 4 came within one review of shipping four differences between them.
  ...
  return (
    ...
    <div
      onClickCapture={(event) => {
        ...
        // The same string `LinkInterceptor` hands to `navigate()` — literally, now — so the
        // actions panel shows the route the app would take, not the raw (possibly relative) href.
        navigate(routeTargetOf(anchor))
      }}
    >
      {children}
    </div>
